...
const lucideIcons = Array.from(icons).filter(i => i.startsWith('lucide:')).map(i => i.replace('lucide:', ''));
...
https.get(url, (res) => {
  ...
  res.on('end', () => {
    ...
    const missing = lucideIcons.filter(i => !found.includes(i) && !(json.aliases && json.aliases[i]));

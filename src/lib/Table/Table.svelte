<script lang="ts" module>import type { TableProps } from './table.types.js';
...
function toggleSort(key: string, event: MouseEvent | undefined = undefined) {
...
const allVisibleSelected = $derived(selection === 'multiple' &&
...
const pinnedVariantSlots = $derived(tableVariants({ pinned: true } as Parameters<typeof tableVariants>[0]));

...
describe('Kbd utilities', () => {
    describe('resolveKey', () => {
        it('should resolve static key symbols', async () => {
        ...
        it('should resolve arrow keys', async () => {
        ...
        it('should preserve unknown keys as-is', async () => {
            expect(resolveKey('A')).toBe('A')
            expect(resolveKey('1')).toBe('1')
            expect(resolveKey('custom')).toBe('custom')
        })

        it('should return null for undefined/empty values', async () => {
    ...
    describe('formatShortcut', () => {
        it('should format shortcuts with default separator', async () => {
            expect(formatShortcut('ctrl+k')).toBe('Ctrl + k')
            expect(formatShortcut('ctrl+shift+p')).toBe('Ctrl + ⇧ + p')
        })

        it('should format shortcuts with custom separator', async () => {
            expect(formatShortcut('ctrl+k', '-')).toBe('Ctrl-k')
            expect(formatShortcut('ctrl+shift+p', ' → ')).toBe('Ctrl → ⇧ → p')
            expect(formatShortcut('shift+enter', '')).toBe('⇧↵')
        })

        it('should return empty string for undefined/empty', async () => {
    ...
    describe('parseShortcutBinding', () => {
        ...
        it('should handle cmd/command as meta', async () => {
        ...
        it('should parse escape as a standalone key', async () => {
    ...
    describe('matchesShortcut', () => {
        ...
        it('should not match wrong key', async () => {
            const binding = parseShortcutBinding('ctrl+k')
        ...
        it('should match complex modifier combos', async () => {
    ...
    describe('normalizeKey', () => {
        ...
        it('should normalize space', async () => {
            expect(normalizeKey(' ')).toBe('space')
        })

        it('should lowercase regular keys', async () => {
            expect(normalizeKey('A')).toBe('a')
            expect(normalizeKey('Escape')).toBe('escape')
            expect(normalizeKey('ArrowUp')).toBe('arrowup')
        })

        it('should pass through already-normalized keys', async () => {

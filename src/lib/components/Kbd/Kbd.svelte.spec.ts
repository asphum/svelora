...
describe('Kbd', () => {
    ...
    describe('rendering', () => {
        it('should render root element', async () => {
            const { container } = await render(Kbd, { value: 'K' })
            expect(getRoot(container)).not.toBeNull()
        })

        it('should render as kbd by default', async () => {
            const { container } = await render(Kbd, { value: 'K' })
            expect(getRoot(container).tagName).toBe('KBD')
        })

        it('should render as custom element', async () => {
            const { container } = await render(Kbd, { value: 'K', as: 'span' })
        ...
        it('should have uppercase class', async () => {
            const { container } = await render(Kbd, { value: 'K' })
    ...
    describe('key mapping', () => {
        it('should render enter as ↵', async () => {
            const { container } = await render(Kbd, { value: 'enter' })
            expect(container.textContent).toContain('↵')
        })

        it('should render escape as Esc', async () => {
            const { container } = await render(Kbd, { value: 'escape' })
            expect(container.textContent).toContain('Esc')
        })

        it('should render tab as ⇥', async () => {
            const { container } = await render(Kbd, { value: 'tab' })
            expect(container.textContent).toContain('⇥')
        })

        it('should render backspace as ⌫', async () => {
            const { container } = await render(Kbd, { value: 'backspace' })
            expect(container.textContent).toContain('⌫')
        })

        it('should render delete as ⌦', async () => {
            const { container } = await render(Kbd, { value: 'delete' })
            expect(container.textContent).toContain('⌦')
        })

        it('should render space as ␣', async () => {
            const { container } = await render(Kbd, { value: 'space' })
            expect(container.textContent).toContain('␣')
        })

        it('should render shift as ⇧', async () => {
            const { container } = await render(Kbd, { value: 'shift' })
            expect(container.textContent).toContain('⇧')
        })

        it('should render arrowup as ↑', async () => {
            const { container } = await render(Kbd, { value: 'arrowup' })
            expect(container.textContent).toContain('↑')
        })

        it('should render arrowdown as ↓', async () => {
            const { container } = await render(Kbd, { value: 'arrowdown' })
            expect(container.textContent).toContain('↓')
        })

        it('should render arrowleft as ←', async () => {
            const { container } = await render(Kbd, { value: 'arrowleft' })
            expect(container.textContent).toContain('←')
        })

        it('should render arrowright as →', async () => {
            const { container } = await render(Kbd, { value: 'arrowright' })
            expect(container.textContent).toContain('→')
        })

        it('should preserve letter keys as-is', async () => {
            const { container } = await render(Kbd, { value: 'A' })
            expect(container.textContent).toContain('A')
        })

        it('should be case-insensitive for special keys', async () => {
            const { container } = await render(Kbd, { value: 'ENTER' })
            expect(container.textContent).toContain('↵')
        })
    })
    ...
    describe('variants', () => {
        it('should default to outline variant', async () => {
            const { container } = await render(Kbd, { value: 'K' })
        ...
        it('should apply solid variant', async () => {
            const { container } = await render(Kbd, { value: 'K', variant: 'solid' })
        ...
        it('should apply outline variant', async () => {
            const { container } = await render(Kbd, { value: 'K', variant: 'outline' })
            expect(getRoot(container).className).toContain('ring')
            expect(getRoot(container).className).toContain('text-on-surface-variant')
        })

        it('should apply soft variant', async () => {
            const { container } = await render(Kbd, { value: 'K', variant: 'soft' })
        ...
        it('should apply subtle variant', async () => {
            const { container } = await render(Kbd, { value: 'K', variant: 'subtle' })
            expect(getRoot(container).className).toContain('bg-surface-container-highest')
    ...
    describe('colors', () => {
        ...
        it('should apply primary color', async () => {
            const { container } = await render(Kbd, {
                value: 'K',
                color: 'primary',
                variant: 'solid'
            })
    ...
    describe('color x variant', () => {
        it('should apply outline + primary', async () => {
            const { container } = await render(Kbd, {
                value: 'K',
                variant: 'outline',
                color: 'primary'
        ...
        it('should apply soft + primary', async () => {
            const { container } = await render(Kbd, {
                value: 'K',
                variant: 'soft',
                color: 'primary'
        ...
        it('should apply subtle + primary', async () => {
            const { container } = await render(Kbd, {
                value: 'K',
                variant: 'subtle',
                color: 'primary'
        ...
        it('should apply soft + error', async () => {
            const { container } = await render(Kbd, { value: 'K', variant: 'soft', color: 'error' })
    ...
    describe('sizes', () => {
        ...
        it('should apply sm size', async () => {
            const { container } = await render(Kbd, { value: 'K', size: 'sm' })
        ...
        it('should apply lg size', async () => {
            const { container } = await render(Kbd, { value: 'K', size: 'lg' })
    ...
    describe('custom class', () => {
        it('should apply custom class to root', async () => {
            const { container } = await render(Kbd, { value: 'K', class: 'my-kbd' })
    ...
    describe('combined features', () => {
        ...
        it('should render outline + error + sm', async () => {
            const { container } = await render(Kbd, {
                value: 'X',
                color: 'error',
                variant: 'outline',
        ...
        it('should render special key with custom styling', async () => {
            const { container } = await render(Kbd, {
                value: 'enter',
                color: 'primary',
                variant: 'solid'
    ...
    describe('empty value', () => {
        it('should render empty when value is undefined', async () => {
            const { container } = await render(Kbd, {})
            expect(container.textContent?.trim()).toBe('')
        })

        it('should render empty when value is empty string', async () => {
            const { container } = await render(Kbd, { value: '' })
            expect(container.textContent?.trim()).toBe('')
        })
    })
})

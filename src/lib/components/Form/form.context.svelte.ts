...
// ==================== CONTEXT KEY ====================
export const FORM_CONTEXT_KEY = Symbol('svelora:form')
...
// ==================== HELPERS ====================
function matchesName(errorName: string | undefined = undefined, target: string): boolean {
    if (!errorName) return false
    return errorName === target || errorName.startsWith(`${target}.`)
}
function matchesTarget(
    errorName: string | undefined = undefined,
...
function addFormPath(error: FormError, formPath: string | undefined = undefined): FormError {
...
export class FormContext<T = unknown> {
    ...
    constructor(
        opts: FormContextOptions<T>,
        formId: string | number,
        parentCtx: FormContext | undefined = undefined
    ) {
        this.#opts = opts
        this.formId = formId
    ...
    // ==================== ERROR QUERIES ====================
    getErrors(name: (string | RegExp) | undefined = undefined): FormErrorWithId[] {
        if (!name) return this.errors
        return this.errors.filter((e) => matchesTarget(e.name, name))
    }
    setErrors(errs: FormError[], name: (string | RegExp) | undefined = undefined): void {
    ...
    clear(name: (string | RegExp) | undefined = undefined): void {
        if (!name) {
            this.errors = []
     ...
     */
    async submit(sourceEvent: SubmitEvent | undefined = undefined): Promise<void> {
    ...
    #matchesAnyName(error: FormErrorWithId, names: string[]): boolean {
        ...
        for (const n of names) {
            ...
            if (pattern?.test(error.name)) return true
        }
        return false
    }
